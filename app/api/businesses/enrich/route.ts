...
async function callWebhook(placeId: string, businessId: string): Promise<{ ok: boolean; reason?: string }> {
  if (!WEBHOOK_URL) {
...
export async function POST(request: NextRequest) {
  ...
  if (!Array.isArray(rawIds)) {
    return NextResponse.json({ error: "business_ids must be an array of strings" }, { status: 400 })
  }

  const businessIds = rawIds.map((id) => (typeof id === "string" ? id.trim() : null)).filter((id): id is string => Boolean(id))
  const uniqueIds = [...new Set(businessIds)]
  const requested = uniqueIds.length

  if (requested === 0) {
    return NextResponse.json({ requested: 0, skipped_no_place_id: 0, succeeded: 0, failed: 0, failures: [] })
  }

  try {
    const supabase = createSupabaseAdmin()
    const { data: rows, error } = await supabase.from("businesses").select("id, place_id").in("id", uniqueIds)
    ...
    const withPlaceId = (rows ?? []).filter((r): r is { id: string; place_id: string } => Boolean(r.place_id && String(r.place_id).trim()))
    ...
    for (let i = 0; i < withPlaceId.length; i += CONCURRENCY) {
      ...
      await Promise.all(
        chunk.map(async ({ id, place_id }) => {
          ...
          else failures.push({ business_id: id, place_id, reason: result.reason ?? "Unknown error" })
        })
      )
    }
